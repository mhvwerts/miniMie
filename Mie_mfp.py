...
currently for gold spheres in water
...
def Mie_gold_water(d_nm, wavelens_nm):
    ...
    if mat == 'Au':
        E = array([0.64,0.77,0.89,1.02,1.14,1.26,1.39,1.51,1.64,1.76,1.88,2.01,2.13,2.26,2.38,2.50,
        2.63,2.75,2.88,3,3.12,3.25,3.37,3.5,3.62,3.74,3.87,3.99,4.12,4.24,4.36,4.49,4.61,4.74,4.86,
        4.98,5.11,5.23,5.36,5.48,5.6,5.73,5.85,5.98,6.1,6.22,6.35,6.47,6.6])
        n = array([0.92,0.56,0.43,0.35,0.27,0.22,0.17,0.16,0.14,0.13,0.14,0.21,0.29,0.43,0.62,1.04,
        1.31,1.38,1.45,1.46,1.47,1.46,1.48,1.50,1.48,1.48,1.54,1.53,1.53,1.49,1.47,1.43,1.38,1.35,
        1.33,1.33,1.32,1.32,1.30,1.31,1.30,1.30,1.30,1.30,1.33,1.33,1.34,1.32,1.28])
        k = array([13.78,11.21,9.519,8.145,7.150,6.350,5.663,5.083,4.542,4.103,3.697,3.272,
        2.863,2.455,2.081,1.833,1.849,1.914,1.948,1.958,1.952,1.933,1.895,1.866,1.871,1.883,1.898,
        1.893,1.889,1.878,1.869,1.847,1.803,1.749,1.688,1.631,1.577,1.536,1.497,1.460,1.427,1.387,
        ...
        OM0 = 0.333 # collision frequency in Hz/1E+14  # check this?
    
    if mat == 'Ag':
        E = array([0.64,0.77,0.89,1.02,1.14,1.26,1.39,1.51,1.64,1.76,1.88,2.01,2.13,
        2.26,2.38,2.50,2.63,2.75,2.88,3,3.12,3.25,3.37,3.5,3.62,3.74,3.87,3.99,4.12,
        4.24,4.36,4.49,4.61,4.74,4.86,4.98,5.11,5.23,5.36,5.48,5.6,5.73,
        5.85,5.98,6.1,6.22,6.35,6.47,6.6])
        n = array([0.24,0.15,0.13,0.09,0.04,0.04,0.04,0.04,0.03,0.04,0.05,0.06,0.05,
        0.06,0.05,0.05,0.05,0.04,0.04,0.05,0.05,0.05,0.07,0.1,0.14,0.17,0.81,1.13,1.34,
        1.39,1.41,1.41,1.38,1.35,1.33,1.31,1.3,1.28,1.28,1.26,1.25,1.22,1.20,1.18,1.15,
        1.14,1.12,1.10,1.07])
        k = array([14.08,11.85,10.10,8.828,7.795,6.692,6.312,5.727,5.242,4.838,4.483,
        4.152,3.858,3.586,3.324,3.093,2.869,2.657,2.462,2.275,2.07,1.864,1.657,1.419,
        1.142,0.829,0.392,0.616,0.964,1.161,1.264,1.331,1.372,1.387,1.393,1.389,1.378,
        1.367,1.357,1.344,1.342,1.336,1.325,1.312,1.296,1.277,1.255,1.232,1.212])
    ...
    # CALCULATION
    wvln = wavelens_nm*1e-9
    Npts = len(wvln)
    ncmplx_wvln = ncmplx_interpol(1240.0/(1e9*wvln))
    
    r_part = d_sphere/2.0 # radius is half the diameter!    
    # mean-free path corrected indices (vectorial calculation)
    ncmplx_r_wvln = ncmplx_mfpcorr(ncmplx_wvln, r_part*1e9, wvln*1e9, FV, OMP, OM0)    
    Qext = zeros(Npts)
    Qsca = zeros(Npts)
    Qabs = zeros(Npts)
    asy = zeros(Npts)
    for idx in range(Npts):
        xco = (2*pi*n_medium*r_part)/wvln[idx]
        # m = ncmplx_wvln[idx]/n_medium # use bulk dielectric function
        m = ncmplx_r_wvln[idx]/n_medium # use mean-free path corrected function
    ...
    # calculate molar extinction coefficient and scatter efficiency
...
if __name__ == "__main__":
    ...
    print(len(wavelens),len(Qsca),len(Qext))
