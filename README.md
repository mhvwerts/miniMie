...
The Mie calculation code is based on MATLAB code from the [report by C. Mätzler](https://doi.org/10.7892/boris.146550) ported to Python through the [`mie`](https://github.com/clegett/mie) library which has now been integrated into the miniMie code base, as it gives results identical to our own initial Python port of the Mätzler code. It is also possible to use [`miepython`](https://github.com/scottprahl/miepython) as an alternative, external computational engine from within miniMie, if that library is available in the Python environment.
