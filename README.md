...
The Mie calculation code is based on MATLAB code from the [report by C. Mätzler](https://doi.org/10.7892/boris.146550) through the [`mie`](https://github.com/clegett/mie) library which has been integrated and adapted into the miniMie code base, as it gives results identical to our own initial Python port of the Mätzler code. If installed in the Python environment, it is also possible to use [`miepython`](https://github.com/scottprahl/miepython) as an alternative, external computational engine from within miniMie.
...
* Optionally, call the Mie calculation code from [`miepython`](https://github.com/scottprahl/miepython), which is not based on Bohren & Huffman, but on Wiscombe, and may be numerically superior in certain cases. Would require `miepython` as an optional dependency
